...
export default function MessagesPage() {
  ...
  const fetchAllUsers = async () => {
    try {
      ...
      const data = await response.json();
      if (data.success) {
        setAllUsers(data.users.filter(u => u._id !== user.id));
      }
    } catch (error) {
  ...
  const sendMessage = async (e) => {
    e.preventDefault();
    if ((!newMessage.trim() && selectedFiles.length === 0) || !selectedConversation || !user || uploadingFile) return;
  ...
  const startNewChat = (otherUser) => {
    const convId = [user.id, otherUser._id].sort().join('_');
          ...
          <div className="bg-white rounded-lg max-w-4xl w-full max-h-[90vh] flex flex-col">
            <div className="bg-gradient-to-r from-blue-600 to-purple-600 text-white px-6 py-4 rounded-t-lg flex justify-between items-center">
              <div>
                ...
                <p className="text-sm text-blue-100">{(previewFile.size / 1024 / 1024).toFixed(2)} MB</p>
              </div>
              <button
